...
```JavaScript
const balabala = function() {this.name = 'balabala'};
const obj = new balabala();
```
通过new操作，主要实现了以下4个行为
```
1. new balbala()会生成一个新对象。
2. 新对象的prototype会委托到我们balabala.prototype对象上来
3. 新对象会指向balabala函数中的this
4. 如果balabala函数中没有返回别的对象，那么就会返回这个新对象
```
具体用法和理解参考以下栗子
